...
def load_data(filename, small, start_and_end=None):
    with open(filename, "rb") as f:
        ...
        if start_and_end is None:
            ...
            data_loaded = data_loaded[start_and_end[0]:start_and_end[1]]
        return data_loaded


def get_dataloader(filenames, small, shuffle, start_and_end):
    datasets = [load_data(filename, small, start_and_end) for filename in filenames]
    squares = datasets[0]
...
# TRAINING
def run_transfernet(ex_num):
    trained_dataloader = get_dataloader(['square.npy', 'triangle.npy', 'circle.npy', 'random.npy'], small=True,
                                        shuffle=True, start_and_end=None)

    train_transfer_dataloader = get_dataloader(
        ['test_square.npy', 'test_triangle.npy', 'test_circle.npy', 'test_chimaeras_v2.npy'], small=True, shuffle=False,
        start_and_end=[0, 7000])

    big_trained_dataloader = get_dataloader(['square.npy', 'triangle.npy', 'circle.npy', 'random.npy'], small=False,
                                            shuffle=True, start_and_end=None)

    test_transfer_dataloader = get_dataloader(
        ['test_square.npy', 'test_triangle.npy', 'test_circle.npy', 'test_chimaeras_v2.npy'], small=False,
        shuffle=False, start_and_end=[7000, -1])
    net = TransferNet()
    ...
    list_of_lists = [accuracy_square_over_time, accuracy_triangle_over_time, accuracy_circle_over_time, accuracy_chimaera_over_time]
    for epoch in range(2):  # loop over the dataset multiple times

        running_loss = 0.0
        if debug:
            my_dataloader = trained_dataloader
        else:
            my_dataloader = big_trained_dataloader
        for i, data in enumerate(my_dataloader, 0):
            ...
            outputs = net.forward_train(inputs)
    ...
    net.set_transfer()
    for epoch in range(2):  # loop over the test_dataset multiple times

        running_loss = 0.0
        for i, data in enumerate(train_transfer_dataloader, 0):
            # get the inputs; data is a list of [inputs, labels]
            inputs, labels = data
            ...
            # forward + backward + optimize
            outputs = net.forward_transfer(inputs)
            loss = criterion(outputs, labels)
            loss.backward()
            optimizer.step()
            ...
            if i % 20 == 19:  # print every 20 mini-batches
                ...
                with torch.no_grad():
                    ...
                    for data in test_transfer_dataloader:
            ...
            plt.savefig(f'./random/Accuracy_of_experiment_{ex_num}.png')  # remainder: change name after every experiment
    ...
    PATH = f'./random/random_improved_network_{ex_num}.pth'
